...
class TagRepository extends ORM\EntityRepository
{
    /**
    ...
    public function getTagsByFrequency($limit = 10, $fillLimit = true)
    {
        ...
        $rsm->addEntityResult('Zikula\TagModule\Entity\TagEntity', 't');
        $rsm->addFieldResult('t', 'tag', 'tag');
        $rsm->addFieldResult('t', 'slug', 'slug');
        $rsm->addFieldResult('t', 'id', 'id');
        $rsm->addScalarResult('freq', 'freq');
        $sql = 'SELECT t.id, t.tag, t.slug, count(j.tag_entity_tag_id) freq' . ' FROM tag_entity_object_tag_entity_tag j' . ' LEFT JOIN tag_tag t ON j.tag_entity_tag_id = t.id' . ' GROUP BY j.tag_entity_tag_id ORDER BY freq DESC';
        if ($limit > 0) {
            $sql .= " LIMIT {$limit}";
        }
        $tags = $this->_em->createNativeQuery($sql, $rsm)->getResult();
        $result = array();
        if (count($tags) > 0) {
            $weightUnit = $tags[0]['freq'] / 5;
            foreach ($tags as $tag) {
                $result[$tag[0]->getTag()] = array('freq' => $tag['freq'], 'tag' => $tag[0]->getTag(), 'slug' => $tag[0]->getSlug(), 'weight' => $this->getWeight($weightUnit, $tag['freq']));
            }
        }
        ...
        if ($fillLimit) {
            ...
            foreach ($unusedTags as $tag) {
                ...
                if (!isset($result[$tag->getTag()])) {
                    $result[$tag->getTag()] = array('freq' => 0, 'tag' => $tag->getTag(), 'slug' => $tag->getSlug(), 'weight' => '20');
                    $tagsNeeded--;
                }
            }
        }
    ...
    {
        $dql = 'SELECT t from Zikula\TagModule\Entity\TagEntity t';
    ...
    public function getTagsWithCount($limit = 0, $offset = 0, $orderBy = 't.tag', $sortDir = 'ASC')
    {
        ...
        $sql = 'SELECT t.id, t.tag, t.slug, count(j.tag_entity_tag_id) cnt FROM tag_tag t' . ' LEFT JOIN tag_entity_object_tag_entity_tag j' . ' ON t.id = j.tag_entity_tag_id GROUP BY t.id' . " ORDER BY {$orderBy} {$sortDir}";
        if ($limit > 0) {
            $sql .= " LIMIT {$limit}";
        }
        $tags = $this->_em->createNativeQuery($sql, $rsm)->getResult();
